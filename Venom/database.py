...
MONGO_DB = "mongodb+srv://Zadkiel:Zadkiel@zadkiel.gjjvqj0.mongodb.net/?retryWrites=true&w=majority"
mongo_dbb = MongoClient(MONGO_DB)

db_x = mongo_dbb["VenomSpamBot"]
...
async def is_session_in_db(client):
    k = await string.find_one({"_id": client})
    if k:
        return True
    else:
        return False
